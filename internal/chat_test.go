...
func TestFromTelegramBuildsChatMetadata(t *testing.T) {
	...
	settings := ChatFromTelegram(TelegramMessage{Chat: Chat{ID: 123, Title: "title"}}, now)
...
func TestParseRowUsesStoredValues(t *testing.T) {
	...
	assert.Equal(t, storedSettingFromRow(t, row, enabled, Workdays(mask)), settings)
}
...
func TestFromScheduleRowPreservesScheduleDefaults(t *testing.T) {
	...
	assert.Equal(t, []ChatSetting{
		...
		{ChatID: 4, EnableAllJung: true, Workday: Workdays(Mon), HasWorkday: true},
	}, settings)
}
...
func TestBuildUpdatePreservesContractShape(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "off work",
			update: BuildOffWorkUpdate("chat-id-dev", 123, "1800", Workdays(Mon|Fri)),
			want: UpdateExpression{
				TableName:        "chat-id-dev",
				Key:              map[string]any{"chatId": int64(123)},
...
func TestFilterDuePreservesContractDefaults(t *testing.T) {
	rows := []ChatSetting{
		{ChatID: 1},
		dueSetting(2, "1000", Workdays(Mon)),
		dueSetting(3, "1000", Workdays(Tue)),
		dueSetting(4, "1800", Workdays(Mon)),
...
func storedSettingFromRow(t *testing.T, row Row, enabled bool, mask Workdays) ChatSetting {
	t.Helper()
...
func dueSetting(chatID int64, offTime string, mask Workdays) ChatSetting {
	return ChatSetting{
		ChatID:     chatID,
		OffTime:    offTime,
		HasOffTime: true,

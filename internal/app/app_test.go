...
func TestShutdownTimeoutUsesFallback(t *testing.T) {
	...
	assert.Equal(t, time.Second, shutdownTimeout(Options{ShutdownTimeout: time.Second}))
	assert.Equal(t, 5*time.Second, readinessDrain(Options{}))
	assert.Equal(t, time.Second, readinessDrain(Options{ReadinessDrain: time.Second}))
}
...
func newTestApp(httpServer httpRunner, metricsServer httpRunner, worker queueRunner, readiness *atomic.Bool) *runtimeApp {
	return New(
		...
		worker,
		Options{Readiness: readiness, ReadinessDrain: time.Nanosecond, ShutdownTimeout: time.Second},
	)
}

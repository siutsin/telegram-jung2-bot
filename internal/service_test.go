...
func TestJungHelpSendsMarkdownHelp(t *testing.T) {
	...
	assert.Equal(t, HelpMessage("Group"), messenger.text)
	assert.Equal(t, SendMessageOptions{
...
func TestSetOffWorkTimeUsesWorkerInput(t *testing.T) {
	...
	err := service.SetOffWorkTime(context.Background(), SetOffInput{
		ChatID:    123,
		ChatTitle: "Group",
		UserID:    456,
		OffTime:   "1800",
...
func TestSetOffWorkTimeReturnsValidationAndAdminErrors(t *testing.T) {
	...
	err := service.SetOffWorkTime(context.Background(), SetOffInput{ChatID: 123, UserID: 456})
...
func statisticsChatCountUpdate(now time.Time) UpdateExpression {
...
func serviceRows() []StoredMessage {
...
type fakeChatStore struct {
	...
	getErr           error
	saveErr          error
	statisticsUpdate UpdateExpression
	updateErr        error
	updatedChatID    int64
}
...
func (store *fakeChatStore) Get(ctx context.Context, tableName string, chatID int64) (ChatSetting, bool, error) {
	if store.getErr != nil {
		return ChatSetting{}, false, store.getErr
	}
	if store.enabled == nil {
		return ChatSetting{}, false, nil
	}

	return ChatSetting{EnableAllJung: *store.enabled}, true, nil
}
...
func (store *fakeChatStore) Update(ctx context.Context, request UpdateExpression) error {
...
func (client *fakeMessageClient) QueryByChat(ctx context.Context, tableName string, chatID int64, since time.Time) ([]StoredMessage, error) {
	if client.err != nil {
		return nil, client.err
	}

	return append([]StoredMessage(nil), client.rows...), nil
}
...
func (messenger *fakeMessenger) SendMessageWithOptions(ctx context.Context, chatID int64, text string, options SendMessageOptions) error {
	messenger.chatID = chatID
	messenger.text = text

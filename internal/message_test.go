...
func TestStoredMessageFromTelegram(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			input: TelegramMessage{
				Chat: Chat{ID: -4, Title: "title"},
				From: &User{ID: 3, UserName: "username", FirstName: "first_name", LastName: "last_name"},
			},
			want: StoredMessage{
				ChatID:      -4,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			assert.Equal(t, test.want, StoredMessageFromTelegram(test.input, now))
		})
	}
}
...
func mustParseMessageTime(t *testing.T) time.Time {
	t.Helper()

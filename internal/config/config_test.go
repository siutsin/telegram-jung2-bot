	...
	tests := []struct {
	...
	}{
		{
			...
			check: func(t *testing.T, config Config) {
				...
				assert.Equal(t, "127.0.0.1:9090", config.MetricsServerAddress)
		...
		},
		{
			...
			mutate: func(env map[string]string) {
				...
				env["METRICS_SERVER_ADDRESS"] = ":9090"
			...
			check: func(t *testing.T, config Config) {
				...
				assert.Equal(t, ":9090", config.MetricsServerAddress)
		...
		},
		{
			...
			check: func(t *testing.T, config Config) {
				...
				assert.Equal(t, "0.0.0.0:9090", config.MetricsServerAddress)
			},
		},
		{
...
func TestServerAddressIgnoresFalseDockerFlag(t *testing.T) {
	...
	assert.Equal(t, "127.0.0.1:9090", metricsServerAddress("", "false"))
}

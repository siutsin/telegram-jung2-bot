...
const stubSetOffFromWorkMessageEvent = require('./stub/onSetOffWorkTimeSQSEvent')
...
test.serial('onEvent - setOffFromWorkTimeUTC', async t => {
  nock(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}`)
    .persist()
    .post('/sendMessage')
    .reply(200, stubAllJungMessageResponse)
  nock(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}`)
    ...
    .query({ chat_id: -123 })
    .reply(200, stubGetChatAdministratorsResponse)
  const clone = JSON.parse(JSON.stringify(stubSetOffFromWorkMessageEvent))
  const sqs = new SQS()
  const response = await sqs.onEvent(clone)
  t.is(response, stubDeleteMessage)
})
